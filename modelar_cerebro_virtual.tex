...
\begin{frame}{Modelización matemática: quimiotaxis}
  ...
  unicelulares (\structure<1>{células}) o pluricelulares dirigen sus movimientos como respuesta a un
    \alert<1>{estímulo químico}»
\end{quote}

\bigskip
Ecuaciones (clásicas) de Keller-Segel: hallar $u(x,t)$, \structure<1>{células}, y
$v(x,t)$, \alert<1>{estímulo químico}, solución del sistema de EDP:
\begin{align*}
  u_t - \Delta u + \alert<2>{\nabla\cdot (u \nabla v)} &= 0,
  \\
  v_t - \Delta v + v - u &= 0,
  \\
  + \text{ condiciones }&\text{iniciales y de contorno.}
\end{align*}
\begin{itemize}
...
\begin{frame}{Modelo de migración de neuroblastos hacia OB}
  \bigskip
    \begin{flushright}
    \includegraphics[width=0.4\linewidth]{img/neuroblastos-roedor-anotado}
    \end{flushright}
    $$
